...
class TestChatSessionManager:
    def test_initial_status(self):
        ...
        assert status["active_provider"] in ("mock", "ollama")
    ...
    def test_process_chat_coding_task(self):
        manager = ChatSessionManager()
        manager.update_settings({"provider": "mock"})
...
class TestWebServerLiveEndpoints:
    ...
    def setup_class(cls):
        ...
        AgentIDERequestHandler.session_manager.update_settings({"provider": "mock", "model": "mock-developer-v1"})
    ...
    def test_api_chat_effective_tokens(self):
        url = f"http://{self.host}:{self.port}/api/chat"
        payload = json.dumps({"prompt": "build 100M token chunked game"}).encode("utf-8")
        req = urllib.request.Request(
            url,
            data=payload,
            headers={"Content-Type": "application/json"},
        ...
        with urllib.request.urlopen(req, timeout=5.0) as resp:
            ...
            assert data["effective_tokens"] >= 100_000_000

    def test_api_chat_stream_endpoint(self):
        url = f"http://{self.host}:{self.port}/api/chat/stream"
        payload = json.dumps({"prompt": "def add(a, b): return a + b"}).encode("utf-8")
        req = urllib.request.Request(
            url,
            data=payload,
            headers={"Content-Type": "application/json"},
        ...
        with urllib.request.urlopen(req, timeout=5.0) as resp:
            ...
            # Last line should be final payload
            final_obj = json.loads(data_lines[-1].replace("data: ", ""))
            assert final_obj["type"] == "final"
            assert "data" in final_obj
